...
bioassay data sources plus the helper resources used for identifier
translation and (for genome-wide runs) the gene anchor. License/
redistribution posture matters for reuse - see each source's own section
for detail, and note that `buildTtdDb()`/`buildBroadRepurposingHubDb()`/
`buildGtoPdbDb()`/`buildUnichemDb()` only ever download into your own
local cache; the package itself redistributes none of this data.
...
A mechanism of action describes the molecular action of a **drug** -
what it does and, almost always, to which molecular target
(`"Bcr/Abl fusion protein inhibitor"`). It is a property of the drug,
not of a drug-target edge, and the data bears that out: 48% of distinct
ChEMBL MOA terms are used by more than one drug ("Histamine H1 receptor
antagonist" covers 55 of them), so a MOA is a shared functional
descriptor rather than a per-edge fact.

That gives two relations rather than one flat table:

- `assembleMoaTable()` - the drug's MOA terms, one row per drug x
  source x MOA term.
- `assembleMoaTargets()` - the separate link from a MOA to the target(s)
  a source says it acts through.

## Which sources carry MOA

Only **ChEMBL, Open Targets and the Broad Repurposing Hub**. TTD, GtoPdb
and DGIdb label a column `MOA`/`type`/`interaction_types`, but what they
store is an *action type* - the verb alone, with no target named. The
distinct-value counts settle it, and ChEMBL settles it at schema level
by carrying both concepts as separate fields:

```{r moa_sources, eval=TRUE}
listMoaSources()[, c("source", "column", "kind", "n_distinct", "used_for_moa")]
```

ChEMBL's `mechanism_of_action` has ~1,589 distinct values; its
`action_type` has 33. TTD's "MOA" column has 48 - it is a clone of the
*action type* field, not of the MOA field. Pooling those under a MOA
heading would let `"Inhibitor"` dominate any grouping or frequency
count, so they are excluded here. Nothing is lost:
`combineDrugTargets()`'s `action` column already carries that dimension
for all six sources (see [Combining Results]).

Mode of Action - the broader cellular or organism-level consequence, as
distinct from MOA - is deliberately not modelled, because no source
carries it systematically: 94% of ChEMBL's MOA strings are
[target] + [action], and the remainder are still molecular or modality
(`"Iron chelating agent"`, `"Surfactant agent"`). The only
MoA-flavoured content anywhere is ChEMBL's free-text `mechanism_comment`
("Role in regulating gastric secretion"), carried through as an optional
passthrough column rather than modelled as a concept.
...
ChEMBL records "Bcr/Abl fusion protein inhibitor" against *two* targets
(ABL1 and BCR), but the drug has one such mechanism - so it appears once
above, and twice in the link table:
...
ChEMBL populates `target_uniprot` and Open Targets `target_symbol` -
ChEMBL's REST output has no gene-symbol column at all. Pass
`resolveGeneSymbol = TRUE` to fill the gap in one batched UniProt
lookup, exactly as `combineDrugTargets()` does:
...
The Broad Hub states its MOA for the drug with no per-target
attribution, so it contributes to the MOA table but **no rows at all**
to the link table - which is why the two relations are separate rather
than one table with drug-level rows flagged inside it.

`moaWide()` collapses the MOA table to one row per drug, and
`queryMoa()` is the one-call wrapper (`queryDrugTargets()` then
`assembleMoaTable()`); call the two separately when you also want the
target link. One caveat when starting from a compound **name**:
`queryDrugTargets()` reaches ChEMBL and Open Targets only via a ChEMBL
ID, so those two need `unichemDbPath` (see [Compound IDs]) for a
name-based query.

MOA strings are kept **verbatim** - only `action_type` is normalized,
since those vocabularies are near-identical once case is folded while
genuine synonymy across free-text prose would need an ontology. Rows are
deduplicated but never merged across sources: the same mechanism from
ChEMBL and Open Targets stays as two rows, because collapsing them would
require a cross-source identity claim this package does not make.
