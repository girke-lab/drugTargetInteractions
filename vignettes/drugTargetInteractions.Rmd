...
covers six independent drug-target data sources - [ChEMBL](https://www.ebi.ac.uk/chembl/)
...
the [Therapeutic Target Database (TTD)](https://ttd.idrblab.cn/) and the
[Broad Institute Drug Repurposing Hub](https://repo-hub.broadinstitute.org/repurposing)
(CLUE) - each queried through its own live REST/GraphQL API or, for TTD and
the Repurposing Hub, a small local SQLite built from public flat files.
ChEMBL remains additionally available through a downloaded local SQLite
instance for users who prefer that (see the [Supplement]).

Five of these (ChEMBL, DGIdb, Open Targets, TTD, Broad Repurposing Hub)
provide curated **drug-target annotations** - an expert-curated call that a
drug acts on a target via some mechanism - and are queried the same way via
...
```{r broad_build, eval=TRUE}
brhDbPath <- buildBroadRepurposingHubDb(rerun = FALSE)  # builds once, reuses the cached file afterward
```
...
**Known operational caveat:** `repo-hub.broadinstitute.org` has been
observed serving an incomplete TLS certificate chain (missing the
InCommon intermediate certificate). Browsers usually mask this by
auto-fetching the missing intermediate; `curl`/R's default download
methods generally do not, so `downloadBroadRepurposingHub()` may fail with
a certificate verification error on some machines until the Broad
Institute fixes their server configuration. This is a server-side issue,
...
## TTD and Broad Repurposing Hub each need a local db path (see above);
## other sources ignore the ones they don't use.
res_all_live <- queryDrugTargets(
    list(molType = "gene", idType = "symbol", ids = "FGFR1"),
    sources = c("chembl", "dgidb", "opentargets", "ttd", "broad"),
    ttdDbPath = ttdDbPath, brhDbPath = brhDbPath)
...
None of the six sources support unlimited-size ID batches in a single
...
sizes automatically - including `queryDrugTargets()`, for the five
